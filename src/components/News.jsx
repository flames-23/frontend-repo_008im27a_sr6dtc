...
export default function News({ lang }) {
  ...
  const seed = async () => {
    try {
      await fetch(`${baseUrl}/api/news/seed`, { method: 'POST' })
      fetchNews()
    } catch (e) {
      setError(e.message)
    }
  ...
  return (
    <section id="news" className="py-20 bg-gradient-to-b from-transparent to-black/[0.02] dark:to-white/[0.03]">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((n, idx) => (
              <motion.article
                ...
                className="group rounded-xl overflow-hidden border border-black/5 dark:border-white/10 bg-white/70 dark:bg-black/40 backdrop-blur hover:shadow-xl hover:shadow-orange-500/10 transition"
              >
                {n.image_url && (
                  <div className="aspect-[16/9] overflow-hidden">
                    <img src={n.image_url} alt="news" className="w-full h-full object-cover group-hover:scale-105 transition" />
                  </div>
                )}
                <div className="p-5">
